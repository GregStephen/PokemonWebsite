...
function showPokemon(pokemonArray) {
	...
	for(let i of pokemonArray) {
		...
		if (i.name.toLowerCase().includes(searchVal)) {
			divOutput.append(`<li class = "pokemonSheet col-4"><figure><a href="#"><img src="${i.image}"></a></figure>
				<div class = "pokemonInfo><p class="id"><span class = "number">#</span>${i.number}</p><h5>${i.name}</h5>`).children(':last').hide().fadeIn(1500);
